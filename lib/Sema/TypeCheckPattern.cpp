...
bool TypeChecker::coercePatternToType(Pattern *&P, DeclContext *dc, Type type,
  ...
  case PatternKind::Is: {
    ...
    if (auto bridgedStoredNSErrorProtocol =
            Context.getProtocol(KnownProtocolKind::BridgedStoredNSError)) {
      conformsToProtocol(castType, bridgedStoredNSErrorProtocol, dc,
    ...
    // FIXME: COPY-PASTE
      if (auto errorCodeProto = Context.getProtocol(
        ...
        if (conformsToProtocol(
                castType, errorCodeProto, dc,
                ConformanceCheckFlags::SuppressDependencyTracking,
                &conformance) &&
            conformance) {
          if (Type errorType = ProtocolConformance::getTypeWitnessByName(
                                 castType, conformance, Context.Id_ErrorType, this)) {
    if (auto bridgedStoredNSErrorProtocol =
            Context.getProtocol(KnownProtocolKind::BridgedStoredNSError)) {
      conformsToProtocol(errorType, bridgedStoredNSErrorProtocol, dc,
                         ConformanceCheckFlags::Used);
    }
